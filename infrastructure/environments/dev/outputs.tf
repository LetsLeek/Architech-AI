...
output "backend_api_key" {
  ...
  sensitive   = true
  description = "AIW-185: the real X-API-Key value for this environment - read via `terraform output -raw backend_api_key` by the deploy workflow's own post-deployment smoke test step, and manually by whoever sets VITE_API_KEY_DEV for the frontend build."
}
